...
function inputDate() {
	...
	if (!isNaN(ngay) && !isNaN(thang) && !isNaN(nam)){
		...
		if (kiemTraSoNgay == 1 && ngay < 31 && nam > 1900 && nam <2048){
			var getTable = document.getElementById('calendar');
			getYear();
			getMonth();
			change(thang-1, nam);
			year.options[nam-1900].selected = 'selected';
			month.options[thang-1].selected = 'selected';
			if (getTable.style.display == 'none' || getTable.style.display == ''){
				getTable.style.display = 'table';
			}	
		} else if (kiemTraSoNgay == 2 && ngay < 32 && nam > 1900 && nam <2048 ){
			var getTable = document.getElementById('calendar');
			getYear();
			getMonth();
			change(thang-1, nam);
